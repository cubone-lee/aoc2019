...
one of their Mars rovers, and so they are curious if you would spend a brief sojourn 
on Mars. You land your ship near the rover.

When you reach the rover, you discover that it's already in the process of rebooting! 
It's just waiting for someone to enter a BIOS password. The Elf responsible for the rover
 takes a picture of the password (your puzzle input) and sends it to you via the Digital 
 Sending Network.
...
--- Part Two ---
Now you're ready to decode the image. The image is rendered by stacking the layers a
nd aligning the pixels with the same positions in each layer. The digits indicate the 
...
if a given position has a transparent pixel in the first and second layers, a black pixel 
in the third layer, and a white pixel in the fourth layer, the final image would have a black
...
with open('input/day8.txt', 'r') as challenge_input:
    pixels = challenge_input.read()


def get_layers(pixels, wide=3, tall=2):
    layers = []
    while pixels:
        layers.append(pixels[:wide * tall])
        pixels = pixels[wide * tall:]
    return layers
...
def render(layers, wide=2):
    ...
    while decoded:
        for i in range(wide):
            image += decoded[i]
        image += '\n'
        decoded = decoded[wide:]

    return image.replace('0', ' ').replace('1', '#')

layers = get_layers(pixels, wide=25, tall=6)
print(part1(layers))
print(render(layers, wide=25))
